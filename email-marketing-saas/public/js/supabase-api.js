 ...
 *
 * Three jobs:
 *  1. Pre-declare window.App so features.js doesn't crash before app.js loads.
 *  2. Redirect Socket.io to the Render backend (not GitHub Pages).
 *  3. Route /api/ calls:
 *       - CRUD data  → Supabase directly (fast, no server round-trip)
 *       - Scraping / AI / Email / WhatsApp → Render backend
 */

// ── Pre-declare App so features.js top-level code doesn't crash ───────────────
// features.js runs `App._scrapedCampaignId = null` at parse time, before
// app.js has loaded. We create a temporary window.App so that line doesn't
// throw ReferenceError. app.js will overwrite it with the real object.
...
(function () {
  ...
  // ── Fix Socket.io: redirect io() to Render, not GitHub Pages ─────────────
  // initSocket() in app.js calls io(window.location.origin) which would try
  // to WebSocket into darapet.github.io — we intercept and send it to Render.
  ...
  // Direct Supabase REST (user JWT keeps RLS enforced)
  async function sbFetch(path, options) {
    options = options || {};
    var t = tok();
    var res = await fetch(SUPABASE_URL + path, Object.assign({}, options, {
      headers: Object.assign(
        { 'Content-Type': 'application/json', 'apikey': window.__LF_ANON_KEY || '' },
  ...
  // Forward to Render (scraping / AI / email / WhatsApp)
  ...
  window.apiFetch = async function (url, options) {
    ...
    // ── RENDER routes (need server-side logic) ─────────────────────────────
    if (clean === '/campaigns' && method === 'POST') return renderFetch(clean, options);
    if (clean.startsWith('/ai/'))           return renderFetch(clean, options);
    if (clean.startsWith('/outreach/'))     return renderFetch(clean, options);
    if (clean.startsWith('/whatsapp/'))     return renderFetch(clean, options);
    if (clean.startsWith('/user/brevo'))    return renderFetch(clean, options);
    if (clean.startsWith('/schedule/send')) return renderFetch(clean, options);

    // ── SUPABASE routes (direct, fast) ─────────────────────────────────────
    ...
    if (clean === '/user/profile' && method === 'PUT') {
      ...
      var up = {}; Object.keys(body||{}).forEach(function(k){if(allowed.indexOf(k)!==-1)up[k]=body[k];});
      await sbFetch('/rest/v1/profiles?id=eq.'+uid(), { method:'PATCH', headers:{'Prefer':'return=minimal'}, body:JSON.stringify(up) });
      return { message: 'Profile updated.' };
    }

    // SMTP / API keys
    if (clean === '/user/smtp' && method === 'PUT') {
      var sa = ['active_smtp','brevo_api_key','sendgrid_api_key','mailgun_api_key','mailgun_domain','smtp_host','smtp_port','smtp_user','smtp_pass','smtp_secure'];
      var su = {}; Object.keys(body||{}).forEach(function(k){if(sa.indexOf(k)!==-1)su[k]=body[k];});
      await sbFetch('/rest/v1/profiles?id=eq.'+uid(), { method:'PATCH', headers:{'Prefer':'return=minimal'}, body:JSON.stringify(su) });
    ...
    if (clean === '/campaigns' && method === 'GET') {
      ...
      return { campaigns: (Array.isArray(data)?data:[]).map(function(c){
        return Object.assign({},c,{campaignId:c.id,leadsCount:c.total_leads||0,createdAt:c.created_at});
      })};
    }

    // Single campaign
    var cmGet = clean.match(/^\/campaigns\/([^/]+)$/);
    if (cmGet && method === 'GET') {
      var sb = getSB();
      var cr = await sb.from('campaigns').select('*').eq('id',cmGet[1]).single();
      var lr = await sb.from('leads').select('*').eq('campaign_id',cmGet[1]);
      return Object.assign({},cr.data,{leads:lr.data||[]});
    }

    // Campaign leads
    var ldM = clean.match(/^\/campaigns\/([^/]+)\/leads$/);
    if (ldM) {
      var ld = await sbFetch('/rest/v1/leads?campaign_id=eq.'+ldM[1]+'&user_id=eq.'+uid()+'&select=*&order=created_at.desc');
    ...
    var tmM = clean.match(/^\/templates\/([^/]+)$/);
    if (tmM && method === 'PUT') {
      var sb3 = getSB(), ta = ['name','subject','body','logo_url','signature_url'], tu = {};
      Object.keys(body||{}).forEach(function(k){if(ta.indexOf(k)!==-1)tu[k]=body[k];});
      var tr = await sb3.from('email_templates').update(tu).eq('id',tmM[1]).eq('user_id',uid()).select().single();
      if (tr.error) throw new Error(tr.error.message);
      return { template: tr.data };
    }
    if (tmM && method === 'DELETE') {
      var sb4 = getSB();
      var td2 = await sb4.from('email_templates').delete().eq('id',tmM[1]).eq('user_id',uid());
      if (td2.error) throw new Error(td2.error.message);
      return { message: 'Deleted.' };
    }
    var dfM = clean.match(/^\/templates\/([^/]+)\/default$/);
    if (dfM && method === 'PUT') {
      var sb5 = getSB();
      await sb5.from('email_templates').update({is_default:false}).eq('user_id',uid());
      await sb5.from('email_templates').update({is_default:true}).eq('id',dfM[1]).eq('user_id',uid());
    ...
    var scM = clean.match(/^\/schedule\/([^/]+)$/);
    if (scM && method === 'DELETE') {
      var sb7 = getSB();
      await sb7.from('scheduled_sends').delete().eq('id',scM[1]).eq('user_id',uid());
      return { message: 'Deleted.' };
    }
    ...
    if (clean === '/admin/stats' && method === 'GET') {
      ...
      return { stats:{total_users:rs[0].count,premium_users:rs[1].count,total_campaigns:rs[2].count,total_emails_sent:rs[3].count} };
    }
    ...
    var adPM = clean.match(/^\/admin\/users\/([^/]+)\/plan$/);
    if (adPM && method === 'PUT') {
      var sb9 = getSB();
      var ap = await sb9.from('profiles').update({plan:body.plan}).eq('id',adPM[1]);
      if (ap.error) throw new Error(ap.error.message);
      return { message: 'Plan updated.' };
    }
    var adAM = clean.match(/^\/admin\/users\/([^/]+)\/admin$/);
    if (adAM && method === 'PUT') {
      var sb10 = getSB();
      var adm  = await sb10.from('profiles').update({is_admin:body.is_admin}).eq('id',adAM[1]);
      if (adm.error) throw new Error(adm.error.message);
      return { message: 'Admin updated.' };
    }

    // Unknown → try Render as fallback
    console.warn('[supabase-api] Unknown route, forwarding to Render:', method, clean);
    return renderFetch(clean, options);
  };

  console.log('[LeadForge] Ready — data:Supabase, server:Render, socket:Render');
})();
